...
@section('content')
    <!-- Dplay 视频播放插件-->
    <link rel="stylesheet" href="{{asset(__STATIC_WEB__)}}/assets/dplay/DPlayer.min.css">
            ...
            <div class="col-sm-3">
                ...
                <div class="col-sm-12 m-2">
                    @foreach($tags as $k => $tag)
                        <label class="badge badge-pill badge-{{Arr::random($tag_color)}}">{{$tag->tag_name}}</label>
                    @endforeach
                </div>
                <blockquote class="blockquote text-left">
                    <p class="mb-0">{{$videos->video_describe}}</p>
                    <footer class="blockquote-footer text-right">{{date_conversion($videos->created_at)}}</footer>
                </blockquote>
    ...
    <input type="hidden" name="video_url" value="{{$videos->video_link}}">
    <script>
        $.ajaxSetup({
            headers: {
